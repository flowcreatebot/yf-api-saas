...
def test_webhook_checkout_completed_falls_back_to_customer_lookup_without_metadata(monkeypatch):
    import app.routes.billing as billing
    from app.db import SessionLocal
    from app.models import APIKey, Subscription, User
    ...
    customer_id = f"cus_fallback_{uuid4().hex[:8]}"
    subscription_id = f"sub_fallback_{uuid4().hex[:8]}"
...
def test_webhook_checkout_completed_no_payment_required_with_client_reference_id_provisions_trialing_key_idempotently(monkeypatch):
    import app.routes.billing as billing
    from app.db import SessionLocal
    from app.models import APIKey, Subscription, User
    ...
    subscription_id = f"sub_client_ref_trial_{uuid4().hex[:8]}"
...
def test_webhook_subscription_created_trialing_provisions_first_key_idempotently(monkeypatch):
    import app.routes.billing as billing
    from app.db import SessionLocal
    from app.models import APIKey, Subscription, User
    ...
    subscription_id = f"sub_created_{uuid4().hex[:8]}"
    customer_id = f"cus_created_{uuid4().hex[:8]}"
...
def test_webhook_subscription_created_active_provisions_first_key_idempotently(monkeypatch):
    import app.routes.billing as billing
    from app.db import SessionLocal
    from app.models import APIKey, Subscription, User
    ...
    subscription_id = f"sub_created_active_{uuid4().hex[:8]}"
    customer_id = f"cus_created_active_{uuid4().hex[:8]}"
...
@pytest.mark.e2e
def test_subscription_updated_trialing_provisions_key_and_enables_market_access(monkeypatch):
    import app.routes.billing as billing
    import app.routes.market as market
    ...
    email, session_token = _register_customer()
    customer_id = f"cus_trialing_{uuid4().hex[:10]}"
    subscription_id = f"sub_trialing_{uuid4().hex[:10]}"
    key_suffix = f"trialing_{uuid4().hex}"
    ...
    class DummyCheckoutSession:
        @staticmethod
        def create(**kwargs):
            return {'id': 'cs_trialing_123', 'url': 'https://checkout.stripe.test/session', 'status': 'open'}
    ...
    class SubscriptionUpdatedTrialingWebhook:
        @staticmethod
        def construct_event(payload, sig_header, secret):
            return {
                'type': 'customer.subscription.updated',
                'data': {
                    'object': {
                        ...
                        'customer': customer_id,
                        'status': 'trialing',
                        'current_period_end': 1769500000,
                    }
                },
            }

    monkeypatch.setattr(billing.stripe, 'Webhook', SubscriptionUpdatedTrialingWebhook)
